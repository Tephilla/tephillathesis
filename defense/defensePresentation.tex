...
\begin{frame}
    ...
    \begin{columns}[T] % align columns
        ...
        \begin{column}{.6\textwidth}
            \begin{itemize}
                \item In BMC, ``We ask whether the system $M$ has any counterexample of length $k$ to (property) $\psi$. This bounded problem is encoded into SAT.'' - A.Biere
                \item In $2D$-BMC, we ask whether the system $M$ has any counterexample of length $\lambda$ and number of clients $\kappa$ to $\psi$ and encode this bounded problem into SMT.
            \end{itemize}
        \end{column}%	
    \end{columns}
\end{frame}
...
\end{frame}
\begin{frame}\frametitle{Challenge 2: Modelling an Unbounded Client-Server (UCS)}
...
\begin{frame}\frametitle{Challenge 3: Modelling an Unbounded Client-Server (UCS)}
