...
\title{Formal verification of systems with \textcolor{purple}{unbounded} agents}


\author[ Tephilla Prince]{Tephilla Prince}
...
\begin{frame}{What is model checking?}
    \begin{center}
        ...
        Does  \includegraphics[width=10em]{figures/softwareio} satisfy \includegraphics[width=2em]{figures/requirements} ?

        \includegraphics[width=2em]{figures/updownarrow}

        Does  \includegraphics[width=5em]{figures/SchedulerPN} satisfy \includegraphics[width=5em]{figures/spec1} ?
    \end{center}
\end{frame}
...
\begin{frame}\frametitle{Challenge 1: Modelling UCS \colorbox{green!30}{using Petri Nets}}
    ...
    \begin{columns}[T] % align columns
        ...
        \hfill
        \begin{column}{.5\textwidth}
            Let $t_c$ be the set containing all token counters.
            \small{
                \begin{align*}
                    t_c
                     & =\{\#p_{PR}, ~\#p_{OP},~\#p_{PU},      \\
                     & \qquad~\#p_{ES},~\#p_{EU},~ \#p_{SR},  \\
                     & \qquad~\#p_{SB}, ~\#p_{RG},~ \#p_{RR}, \\
                     & \qquad~\#p_{DP}\}
                \end{align*}
            }
        \end{column}%
...
\begin{frame}{Linear Temporal Logic with Integer Arithmetic {\LC}}
    ...
    In {\LC}, at the propositional level, we allow token counters and arithmetic operators over them.

    Let $t_c$ be the set containing all token counters. The formulas in {\LC} are given as follows:
    \begin{equation}
        \begin{split}
    ...
    \noindent where $\#p \in t_c$ and $\mathfrak{c} \in \mathbb{Z}^+$.


    \[\psi::= \beta \mid \lnot \psi  \mid \psi\lor\psi^{\prime}   \mid \psi\land\psi^{\prime}  \mid  X \psi  \mid F \psi\mid G \psi \mid \psi U  \psi^{\prime}\]
    ...
    \begin{columns}[T] % align columns
        \begin{column}{.4\textwidth}
            \begin{figure}
                ...
                \scalebox{0.5}{\input{4_unboundedpn}}
        ...
        \begin{column}{.6\textwidth}
            \begin{figure}
                \centering
                \scalebox{0.7}{\input{4_interleaveseq}}
            ...
            \begin{figure}
                \centering
                \scalebox{0.7}{\input{4_concurseq}}

                \caption{Sequence of firings via concurrent semantics}
                \label{fig:concurseq}
            \end{figure}
        \end{column}%
    \end{columns}
...
\begin{frame}\frametitle{Modelling an Unbounded Client-Server (UCS)\footnote{On Verifying Unbounded Client-Server Systems, T.Prince, EUMAS'23}}
    \begin{columns}[T] % align columns
        \begin{column}{.5\textwidth}
            \begin{figure}
                \begin{center}
...
\begin{frame}{List of publications:}
    \begin{enumerate}
        \item[1] Ramchandra Phawade, Tephilla Prince, S. Sheerazuddin:
            Two dimensional bounded model checking for unbounded Petri nets. Arxiv (2026),  submitted to TopNOC
        \item[2] Ramchandra Phawade, Tephilla Prince, S. Sheerazuddin: Verification of Unbounded Client-Server Systems with Distinguishable Clients. Arxiv (2026), submitted to TopNOC
        \item[3]   Tephilla Prince:
            On Verifying Unbounded Client-Server Systems. Springer EUMAS 2023: 465-471
        \item[4]   Francesco Di Cosmo, Tephilla Prince:
...
\begin{frame}{List of publications:}
    \begin{enumerate}
        ...
        \item[6]  Francesco Di Cosmo, Soumodev Mal, Tephilla Prince: Nets-Within-Nets Through the Lens of Data Nets. Reachability Problems 2025
        \item[7] Francesco Di Cosmo, Soumodev Mal, Tephilla Prince: Decidability and Complexity of Reachability and Coverability in Lossy EOS (Yet to be submitted)
...
\begin{frame}{List of tools built}
    \begin{enumerate}
        % \item DCModelChecker verifies PN over counting LTL properties \url{https://doi.org/10.6084/m9.figshare.19875226}
        \item[1] DCModelChecker1.0 verifies PN over {\LC} with interleaved semantics \url{https://doi.org/10.5281/zenodo.7084996}
        \item[2] DCModelChecker2.0 verifies PN over {\LC} allowing concurrent semantics \url{https://doi.org/10.5281/zenodo.7198485}
        \item[3] UCSChecker verifies $\nu$-nets over {\Lstar} properties \url{https://doi.org/10.5281/zenodo.15847737}
        \item[4] NWN Telingo Analyser: verifies lossy and perfect PN and EOSs over LTL properties \url{https://doi.org/10.5281/zenodo.11401876}
    \end{enumerate}
...
\begin{frame}{Experimental Results}
    ...
    \begin{enumerate}
        \item  the correctness of results returned by DCModelChecker 2.0 in comparison with the state-of-the-art ITS-Tools, obtaining tool confidence 0.92.
        \item  the comparison of DCModelChecker 2.0 on FireabilityCardinality properties which are not verifiable by tools in the MCC for various bounds,
        \item verification of unbounded Petri nets in comparison with other tools,
        \item the strength of DCModelChecker 2.0 is in additionally verifying invariants specified in {\LC} which are not verifiable by tools in the MCC nor DCModelChecker 1.0.
    \end{enumerate}
\end{frame}
...
\begin{frame}{\tiny Travel Agency Case Study}
    \begin{figure}
        \centering
        % \resizebox{0.5\linewidth}{!}{\include{figures/fig_travelagencyv0_vnet}}
        \scalebox{0.3}{\include{figures/fig_travelagencyv0_vnet}}
...
\begin{frame}
    \begin{enumerate}
        \item[1] If a client has entered the system, then eventually it will exit the system. This can be expressed as $\Phi_1$=$\mathbf{G}_s(\forall x) \big( p_{il}(x) \Rightarrow \mathbf{F}_c~p_{es}(x) \big)$.
        \item[2]  If a client has entered the system, then it will not be in the place $p_{il}$ after exiting the system. This can be expressed as $\Phi_2$=$\mathbf{G}_s(\forall x) \big( p_{es}(x) \Rightarrow \neg p_{il}(x) \big)$.
        \item[3]  If a client has entered the system and it has chosen the mode of journey as flight, then eventually it will be in the place $p_{cp}$, where they share their payment information. This can be expressed as $\Phi_3$=$\mathbf{G}_s(\forall x) \big( p_{il}(x) \land p_{bf}(x)\Rightarrow \mathbf{F}_c~p_{cp}(x) \big)$.
...
\begin{frame}
    \begin{enumerate}
        \item[4]  If a client has entered the system and it has chosen the mode of journey as flight, and its journey is not yet validated by the server, then eventually it will be in the place $p_{cp}$, where they share their payment information to the server. This can be expressed as $\Phi_4$=$\mathbf{G}_s(\forall x) \big( p_{il}(x) \land p_{bf}(x) \land \neg p_{cp}(x)\Rightarrow \mathbf{F}_c~p_{cp}(x) \big)$.
        \item[5]  If a client has entered the system, then eventually it will be in the place $p_{cp}$, after which it gets server approval. This can be expressed as $\Phi_5$=$\mathbf{G}_s(\forall x) \big( p_{il}(x) \Rightarrow \mathbf{F}_c~p_{cp}(x) \big)$.
        \item[6] If a client's payment has been validated, then it will not be in the place $p_{cp}$. This can be expressed as $\Phi_6$=$\mathbf{G}_s(\forall x) \big( p_{es}(x) \Rightarrow \neg p_{cp}(x) \big)$.
        \item[7] If a client is searching for a journey it can only choose either of the three flight, train or bus journeys. $\Phi_7$=$\mathbf{G}_s(\forall x) \big( p_{ss}(x) \Rightarrow F_c( p_{bf}(x) \lor p_{bt}(x) \lor p_{bb}(x) \big)$.
    \end{enumerate}

\end{frame}
\begin{frame}{Experiments with UCSChecker}
    \begin{figure}
        \begin{center}
            \scalebox{0.3}{\includegraphics{table1}}
        \end{center}
...
\begin{frame}{Experiments with UCSChecker}
    \begin{figure}
        \begin{center}
            \scalebox{0.3}{\includegraphics{table2}}
        \end{center}
