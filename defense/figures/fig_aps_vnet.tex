...
sep=0pt,minimum height=.40cm, minimum width=.5cm] (p1) at (-5,-2){$0$} ;

%places and transitions for request granted scenarios		
		%accept transition
		\node[transition,fill=black,minimum height=.4cm,minimum width=.4cm,label=right:{\small $t_{acc}$}] (t1) at (-3,-4) {};
		
		%server busy place
		%\node[place,label=below:{\small $p_{SB}$},draw=red!50,fill=blue!20,innersep=0pt,minimum height=.40cm, minimum width=.5cm] (p7) at (-7,-4){} ;
		...
		\node[place,label=right:{\small $p_{OP}$},draw=red!50,fill=green!20,inner
sep=0pt,minimum height=.40cm, minimum width=.5cm] (p2) at (-3,-6){} ;	
		
		%server place request granted rg	
		%\node[place,label=below:{\small $p_{RG}$},draw=red!50,fill=green!20,innersep=0pt,minimum height=.40cm, minimum width=.5cm] (p3) at (-5,-6){} ;
		

		%transition exit
		\node[transition,fill=black,minimum height=.4cm,minimum width=.4cm,label=right:{$t_{exit}$}] (t2) at (-3,-8) {};
		
		%client place Exit successfully ES
		\node[place,label=right:{\small $p_{ES}$},draw=red!50,fill=green!20,inner
sep=0pt,minimum height=.40cm, minimum width=.5cm] (p4) at (-3,-10){} ;			

		%server place deallocate parking lot DP
		%\node[place,label=below:{\small $p_{DP}$},draw=red!50,fill=green!20,innersep=0pt,minimum height=.40cm, minimum width=.5cm] (p5) at (-5,-10){} ;
		
		%transition accept sink
		\node[transition,fill=black,minimum height=.4cm,minimum width=.4cm,label=right:{\small $t_{acc\_sink}$}] (t3) at (-3,-12) {} edge[post] node[auto] {2} (p2) ;
...
sep=0pt,minimum height=.40cm, minimum width=.5cm] (p6) at (1.5,-6){} ;
		...
		\node[transition,fill=black,minimum height=.4cm,minimum width=.4cm,label=right:{$t_{rej\_sink}$}] (t5) at (1.5,-8) {};

		%server place request rejected RR
		%\node[place,label=below:{\small $p_{RR}$},draw=red!50,fill=blue!20,innersep=0pt,minimum height=.40cm, minimum width=.5cm] (p8) at (5,-6){} ;
